...
semKITTI_3D_OUT_PATH = '/data2/jy/test_semantick_kitti_downed0.05'
sub_grid_size = 0.05  # 🎈对于SemanticKITTI，ReDAL是0.05m
print(sub_grid_size)#

# 定义validation序列
validation_sequence = '08'
...
class SemKITTIDatasetConverter:
    ...
    def convert_to_bin_and_label(cls, data_path, out_path):
        ...
        # 获取所有序列
        sequences = [f'sequences/{i:02d}' for i in range(11,22)]  #
        for sequence in sequences:

            # 获取该序列中的所有点云文件
            bin_files = sorted(glob.glob(os.path.join(data_path, sequence, 'velodyne/*.bin')))

            # 处理每隔10帧的点云文件，对于序列08每帧都处理
            for idx, bin_file in enumerate(bin_files):
                # if sequence == 'sequences/08':
                #     pass  # 序列08每帧都处理，不需要额外条件
                # elif idx % 10 != 0:
                #     continue
