...
class SemKITTI(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #                    ↓
        # Area_i#点云名称 ={-100 ,-100 , ... ,  i , ... }  i代表该点的标签
        ...
        # 以字典形式返回
        # 返回值：
        # 原始点云其实包含（坐标，颜色等特征），且是经过体素化和稀疏张量化后的数据。
        # lidar_Origin：原始点云
        # targets_Origin：原始点云的标签
        # targets_mapped_Origin：原始点云的标签
        # inverse_map_Origin：原始点云的逆变换
        # lidar_StrongAug：增强后的点云
        # targets_StrongAug：增强后的点云的标签
        # targets_mapped_StrongAug：增强后的点云的标签
        # inverse_map_StrongAug：增强后的点云的逆变换
        # file_name：点云名称
        return {
