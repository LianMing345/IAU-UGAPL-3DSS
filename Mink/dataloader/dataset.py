...
class ScannetDataset:
    ...
    def __init__(self, xyz, colors, label_to_idx, file_names, voxel_size, labels=None, labeled_points=None):
        self.xyz = xyz
        self.colors = colors
        self.labels = labels
    ...
    def __getitem__(self, idx):
        ...
        if self.labels is not None:
            ...
            labels = self.label_to_idx[labels]
        else:
            labels = np.zeros(shape=(len(coords),))
    ...
    def voxelize(self, coords, feats, labels, is_prevoxel_aug, is_postvoxel_aug):
        ...
        # feat_ = np.concatenate([feats, coords], axis=1)
        feat_ = feats
        # feat_ = np.zeros(shape=(len(pc_), 4))
        # feat_[:, :3] = pc_[:, :3]
